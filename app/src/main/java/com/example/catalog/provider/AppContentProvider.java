...
public class AppContentProvider extends ContentProvider {
    ...
    /** URI ID to get all items. */
    private static final int ITEMS = 100;
    /** URI ID to get an item. */
    ...
    private static UriMatcher buildUriMatcher() {
        ...
        matcher.addURI(authority, AppContract.Items.PATH, ITEMS);
        matcher.addURI(authority, AppContract.Items.PATH + "/*", ITEMS_ID);
    ...
    public String getType(Uri uri) {
        ...
        switch (match) {
            case ITEMS:
                return AppContract.Items.CONTENT_TYPE;
            case ITEMS_ID:
                return AppContract.Items.CONTENT_ITEM_TYPE;
            default:
    ...
    public Uri insert(Uri uri, ContentValues values) {
        ...
        switch (match) {
            case ITEMS: {
                db.insertOrThrow(AppDatabase.Tables.ITEMS, null, values);
                getContext().getContentResolver().notifyChange(uri, null);
                // Generate Uri with remote id.
                return AppContract.Items.buildUri(values.getAsString(AppContract.Items._ID));
            }
            default: {
    ...
    private SelectionBuilder buildSimpleSelection(Uri uri, int match) {
        ...
        switch (match) {
            case ITEMS: {
                return builder.table(AppDatabase.Tables.ITEMS);
            }
            case ITEMS_ID: {
                final String id = AppContract.Items.getId(uri);
                return builder.table(AppDatabase.Tables.ITEMS)
                        .where(AppContract.Items._ID + "=?", id);
            }
            default: {
